...
proto._getNodeElById = function (treeEl, indexId, ignoreCache = false) {
  if (indexId == null)
    return null;
  if (!this._cacheEls)
    this._cacheEls = {};
...
proto._getDraggableNodeEl = function (treeEl, ignoreCache = false) {
  if (!this._cacheEls)
    this._cacheEls = {};
...
proto._getPlaceholderNodeEl = function (treeEl, ignoreCache = false) {
  if (!this._cacheEls)
    this._cacheEls = {};
...
proto.moveIndex = function (index, dragInfo, e, canMoveFn, canMoveToCollapsed = false) {
  var newIndex = null;
        ...
        if (trgRect) {
          ...
          availMoves = availMoves.filter(am => {
            ...
            if ((placement == 'before' || placement == 'after') && trg.id == 1)
              return false;
            if (!canMoveToCollapsed && trg.node.collapsed && (placement == 'append' || placement == 'prepend'))
              return false;
